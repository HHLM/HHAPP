...
#import "HHLikeTableView.h"
#import "BaseTableViewCell.h"
@interface HHLikeTableView ()<UITableViewDelegate,UITableViewDataSource>
//是否是多维数组 default YES 多维 , NO 一层
@property (nonatomic, assign) BOOL space;
@end
@implementation HHLikeTableView
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.likeDelegate && [self.likeDelegate respondsToSelector:@selector(likeTableViewCellRowHeight:indexPath:)]) {
        return [self.likeDelegate likeTableViewCellRowHeight:self indexPath:indexPath];
    }
    return 44;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
   
    if (self.likeDelegate && [self.likeDelegate respondsToSelector:@selector(likeTableView:indexPath:)]) {
        return [self.likeDelegate likeTableView:self indexPath:indexPath];
    }
