...
#import "TRBadgeButton.h"
@interface TRBadgeButton()
...
- (void)initializer
{
    ...
    self.badgePadding   = 2;
    self.badgeMinSize   = 4;
    self.badgeOriginX   = 11;
    self.badgeOriginY   = -12;
    self.shouldHideBadgeAtZero = YES;
    self.shouldAnimateBadge = YES;
    // Avoids badge to be clipped when animatingits scale
...
- (void)updateBadgeFrame
{
    ...
    if (self.type == TRBadgeButtonOfShowRedPoint) {
        ...
        if (self.badge.text.length > 1) {
            self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY,24, 20);
        }else {
            self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY,20, 20);
        }
...
- (void)setBadgeValue:(NSString *)badgeValue
{
    ...
    if ( [badgeValue isEqualToString:@""] || ([badgeValue isEqualToString:@"0"] && self.shouldHideBadgeAtZero)) {
        [self removeBadge];
    } else if (!self.badge) {
...
- (void)setType:(TRBadgeButtonType)type {
    _type = type;
}
@end
